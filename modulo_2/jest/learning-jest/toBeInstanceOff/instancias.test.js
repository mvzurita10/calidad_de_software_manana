...
describe('InstanceOf()', () => {
    ...
    test('Happy: esFecha detecta instancias Date válidas', () => {
        ...
        expect(esFecha(new Date('inválido'))).toBe(false); // Date inválida
    });

    test('Sad: crearPersona con datos inválidos', () => {
        expect(() => crearPersona('', 18)).toThrow('nombre inválido');
        expect(() => crearPersona('Luis', -2)).toThrow('edad inválida');
